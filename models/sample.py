...
class SampleOrder(models.Model):
    ...
    def _get_invoiced(self):
        ...
        for order in self:
            if order.state not in ('sample', 'done'):
    ...
    origin = fields.Char('Source Document', copy=False,
        help="Reference of the document that generated this Sample order "
    ...
    state = fields.Selection([
        ('draft', 'Borrador'),
        ('sent', 'Enviado'),
        ('to approve', 'Por Aprobación'),
        ('sample', 'Sample Order'),
        ('done', 'Locked'),
        ('cancel', 'Cancelled')
    ], string='Status', readonly=True, index=True, copy=False, default='draft', tracking=True)
    order_line = fields.One2many('sample.order.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True)
    ...
    def _compute_access_url(self):
        super(SampleOrder, self)._compute_access_url()
        for order in self:
            order.access_url = '/my/Sample/%s' % (order.id)

    @api.depends('state', 'date_order', 'date_approve')
    def _compute_date_calendar_start(self):
        for order in self:
            order.date_calendar_start = order.date_approve if (order.state in ['sample', 'done']) else order.date_order
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            ...
            vals['name'] = self.env['ir.sequence'].next_by_code('sample.order', sequence_date=seq_date) or '/'
        return super(SampleOrder, self).create(vals)

    def unlink(self):
        for order in self:
            if not order.state == 'cancel':
                raise UserError(_('In order to delete a Sample order, you must cancel it first.'))
        return super(SampleOrder, self).unlink()

    def copy(self, default=None):
        ctx = dict(self.env.context)
        ctx.pop('default_product_id', None)
        self = self.with_context(ctx)
        new_po = super(SampleOrder, self).copy(default=default)
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        """
        result = super(SampleOrder, self).onchange(values, field_name, field_onchange)
    ...
    def _get_report_base_filename(self):
        self.ensure_one()
        return 'Sample Order-%s' % (self.name)
    ...
    def onchange_partner_id_warning(self):
        if not self.partner_id or not self.env.user.has_group('Sample.group_warning_Sample'):
            return
        warning = {}
        title = False
        message = False
        ...
        if partner.Sample_warn == 'no-message' and partner.parent_id:
            partner = partner.parent_id

        if partner.Sample_warn and partner.Sample_warn != 'no-message':
            # Block if partner only has warning but parent company is blocked
            if partner.Sample_warn != 'block' and partner.parent_id and partner.parent_id.Sample_warn == 'block':
                partner = partner.parent_id
            title = _("Warning for %s", partner.name)
            message = partner.Sample_warn_msg
            warning = {
                'title': title,
                'message': message
            }
            if partner.Sample_warn == 'block':
    ...
    def action_rfq_send(self):
        '''
        This function opens a window to compose an email, with the edi Sample template message loaded by default
        '''
        self.ensure_one()
        ir_model_data = self.env['ir.model.data']
        try:
            if self.env.context.get('send_rfq', False):
                template_id = ir_model_data.get_object_reference('sample', 'email_template_edi_Sample')[1]
            else:
                template_id = ir_model_data.get_object_reference('sample', 'email_template_edi_Sample_done')[1]
        except ValueError:
            template_id = False
        try:
        ...
        ctx.update({
            'default_model': 'sample.order',
            'active_model': 'sample.order',
        ...
        if self.state in ['draft', 'sent']:
            ...
            ctx['model_description'] = _('Sample Order')
    ...
    def message_post(self, **kwargs):
        ...
        return super(SampleOrder, self.with_context(mail_post_autofollow=True)).message_post(**kwargs)

    def print_quotation(self):
        self.write({'state': "sent"})
        return self.env.ref('Sample.report_Sample_quotation').report_action(self)

    def button_approve(self, force=False):
        self.write({'state': 'sample', 'date_approve': fields.Datetime.now()})
    ...
    def button_confirm(self):
        ...
        for order in self:
            ...
            if order.company_id.po_double_validation == 'one_step'\
                    ...
                    or order.user_has_groups('Sample.group_Sample_manager'):
        ...
        for order in self:
            for inv in order.invoice_ids:
                if inv and inv.state not in ('cancel', 'draft'):
                    raise UserError(_("Unable to cancel this Sample order. You must first cancel the related vendor bills."))
    ...
    def _prepare_invoice(self):
        """Prepare the dict of values to create the new invoice for a Sample order.
        """
        self.ensure_one()
        move_type = self._context.get('default_move_type', 'in_invoice')
        journal = self.env['account.move'].with_context(default_move_type=move_type)._get_default_journal()
        if not journal:
            raise UserError(_('Please define an accounting Sample journal for the company %s (%s).') % (self.company_id.name, self.company_id.id))
    ...
    def action_view_invoice(self, invoices=False):
        ...
        given Sample order ids. When only one found, show the vendor bill
        immediately.
        """
        if not invoices:
            # Invoice_ids may be filtered depending on the user. To ensure we get all
            # invoices related to the Sample order, we read them in sudo to fill the
            # cache.
    ...
    def retrieve_dashboard(self):
        ...
        query = """SELECT COUNT(1)
                   ...
                   JOIN Sample_order po ON (po.id = m.res_id)
                   WHERE m.create_date >= %s
                     AND m.model = 'sample.order'
        ...
        # Calculated values ('avg order value', 'avg days to Sample', and 'total last 7 days') note that 'avg order value' and
    ...
    def _get_orders_to_remind(self):
        """When auto sending a reminder mail, only send for unconfirmed Sample
        order and not all products are service."""
        return self.search([
            ('receipt_reminder_email', '=', True),
            ('state', 'in', ['sample', 'done']),
    ...
    def get_confirm_url(self, confirm_type=None):
        """Create url for confirm reminder or Sample reception email for sending
        in mail."""
    ...
    def get_update_url(self):
        """Create portal url for user to update the scheduled date on Sample
        order lines."""
    ...
    def confirm_reminder_mail(self, confirmed_date=False):
        for order in self:
            if order.state in ['sample', 'done'] and not order.mail_reminder_confirmed:
    ...
    def _confirm_reception_mail(self):
        for order in self:
            if order.state in ['sample', 'done'] and not order.mail_reception_confirmed:
...
class SampleOrderLine(models.Model):
    ...
    order_id = fields.Many2one('sample.order', string='Order Reference', index=True, required=True, ondelete='cascade')
...
#    ]
    ...
    def _prepare_compute_all_values(self):
        ...
        # is not implemented yet on the Sample orders.
    ...
    def create(self, vals_list):
        ...
        for line in lines:
            if line.product_id and line.order_id.state == 'sample':
    ...
    def write(self, values):
        if 'display_type' in values and self.filtered(lambda line: line.display_type != values.get('display_type')):
            raise UserError(_("You cannot change the type of a Sample order line. Instead you should delete the current line and create a new line of the proper type."))
        ...
        return super(SampleOrderLine, self).write(values)

    def unlink(self):
        for line in self:
            if line.order_id.state in ['sample', 'done']:
                raise UserError(_('Cannot delete a Sample order line which is in state \'%s\'.') % (line.state,))
        return super(SampleOrderLine, self).unlink()
    ...
    def _get_date_planned(self, seller, po=False):
        """Return the datetime value to use as Schedule Date (``date_planned``) for
           ...
           :param Model po: sample.order, necessary only if the PO line is
    ...
    def _product_id_change(self):
        ...
        self.name = self._get_product_Sample_description(product_lang)

        self._compute_tax_id()

    @api.onchange('product_id')
    def onchange_product_id_warning(self):
        if not self.product_id or not self.env.user.has_group('Sample.group_warning_Sample'):
            return
        warning = {}
        title = False
        message = False
        ...
        if product_info.Sample_line_warn != 'no-message':
            title = _("Warning for %s", product_info.name)
            message = product_info.Sample_line_warn_msg
            warning['title'] = title
    ...
    @api.model
    def _prepare_Sample_order_line(self, product_id, product_qty, product_uom, company_id, supplier, po):
    ...
    def _track_qty_received(self, new_qty):
        self.ensure_one()
        if new_qty != self.qty_received and self.order_id.state == 'sample':
            self.order_id.message_post_with_view(
                'Sample.track_po_line_qty_received_template',
